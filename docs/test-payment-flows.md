...
## Prerequisites
1. Ensure `payment_SECRET_KEY` is set in Replit Secrets
2. Have test payment API keys (not production keys)
3. Use payment test card numbers for safe testing

## Test Card Numbers (payment Test Environment)
...
3. Enter amount: ₦1000
4. Click "Deposit" - should redirect to payment
...
1. Use payment's webhook test tool
...
curl -X POST http://localhost:5000/api/webhook/payment \
  ...
  -H "X-payment-Signature: test_signature" \
...
### Before Testing
- [ ] Set payment_SECRET_KEY in Secrets
- [ ] Verify using payment test environment
